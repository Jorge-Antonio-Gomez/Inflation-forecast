...
# IN FUNCTION:
   # serie: i.e. ts_inflation
   # modelname: i.e. arima_2
   # order: i.e. c(2,0,0)
   # period: i.e. 12 (1 if you don't want deterministic seasonality)
   # outSampleSize: i.e. 24
iterate_forecast <- function(serie, model_name, order, outSampleSize, period) {
  # Number of forecasts:
  nfcast <- outSampleSize + 1
  
  # Start year of the forecast (using nfcast):
  startYearToFcast <- currentYear - (outSampleSize*12)
  
  # Create a vector to save the forecast:
  paste0("forecast_", modelname) <- rep(NA, nfcast)
  
  # Cicle
  # for (i in 1:(nfcast + 1)) {
  #   inSample <- window(serie, end = c(2022, 3 + i))
  #   model <- Arima(inSample, order = order, seasonal = list(order = c(0,0,0), period = period))
  #   paste0("forecast_", modelname)[i] <- forecast(model, h = 1)$mean
  # }
  # 
  # return(paste0("forecast_", modelname))
}

iterate_forecast(ts_inflation, "rw", c(0,1,0), 1, 12)
