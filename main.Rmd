...
# Rename:
  colnames(exchange_rate) <- c("index", "fecha", "exchange_rate")

# Formato de fecha:
  exchange_rate$fecha <- as.Date(exchange_rate$fecha, format = "%d/%m/%Y")

# Cortar datos desde currentYear:
  exchange_rate <- "a"
  
# Cortar datos hasta currentMonth
  exchange_rate <- filter(exchange_rate, as.numeric(substr(fecha, 1, 4)) < currentYear | as.numeric(substr(fecha, 6, 7)) <= last_month)
...
```
<!-- Para no correr el código al compilar markdown -->
<!-- ```{r, eval = FALSE} -->
```{r}
# LINEAR MODEL
